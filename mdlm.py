"""
Basic MDLM model implementation 
"""
...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def __init__(self, dim, theta=10000.0, max_seq_len=2048):
        ...
        inv_freq = 1.0 / (theta ** (torch.arange(0, dim, 2).float() / dim)) # 1 / (10000^(2i/dim)) for each even index i (1 frequency per dimension)
        self.register_buffer('inv_freq', inv_freq) # store inv_freq as a non-trainable buffer 
    ...
    def forward(self, position_ids):
        ...
        positions = torch.arange(seq_len, device=device).float() # get sequence length + create a tensor of positions
        
        # Compute angles: [seq_len, dim//2]
        # Compute angles by taking outer product of position indices with precomputed inverse frequencies to produce [seq_len, dim//2] 
        freqs = torch.outer(positions, self.inv_freq) # each element freqs[m, i] = m / (theta^(2i/dim)) is the rotation angle for position m at frequency i
...
class TransformerBlock(nn.Module):
    """Single transformer block with bidirectional attention and RoPE."""
    ...
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1, use_rope=True, rope_dim=None):
        ...
        # Feed-forward with SwiGLU (requires 2x the hidden size for the gate projection)
    ...
    def forward(self, x, rotary_pos_emb=None):
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim) # all positions attend to all positions
        ...
        ## Pre-norm residual connection before feed-forward 
        residual = x
        x = self.norm2(x)
        ## SwiGLU activiation: Swish(gate_proj(x)) * up_proj(x)
        gate = self.gate_proj(x) # [batch, seq, d_ff] - gate projection output
        up = self.up_proj(x) # [batch, seq, d_ff] - up projection output
        activated = swiglu(gate, up) # [batch, seq, d_ff] - SwiGLU output
...
class MDLModel(nn.Module):
    """
    Stacks transformer blocks with RoPE, RMSNorm, and SwiGLU to predict masked tokens in sequences. 
    """
    ...
    def forward(self, input_ids, position_ids=None, attention_mask=None):
        """
        Args:
            input_ids: [batch_size, seq_len] - tokens with mask tokens
            position_ids: [batch_size, seq_len] - position indices (optional)
            attention_mask: [batch_size, seq_len, seq_len] - attention mask (optional, not used for bidirectional)
